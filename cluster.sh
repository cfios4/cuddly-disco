...
CONTROL=$(filter_loopbacks "${CNODES[@]}")
WORK=$(filter_loopbacks "${WNODES[@]}")
...
/tmp/talosctl gen config t3s https://${CONTROL[0]} --install-image factory.talos.dev/installer/6bd594353a7860f79cc0910931af93ffd6890ef32aaa01db81eb90c1de2e55e6:v1.9.3 --config-patch-control-plane '{"cluster": {"allowSchedulingOnControlPlanes": true}}'
