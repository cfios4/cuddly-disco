...
//   value       = [for i in range(local.worker_nodes) : proxmox_virtual_environment_vm.worker[i].ipv4_addresses]
// }
...
output "control_plane_ips" {
  description = "IP addresses of the control plane nodes"
  value       = proxmox_virtual_environment_vm.control_plane.ipv4_addresses[1]
}

output "worker_ips" {
  description = "IP addresses of the worker nodes"
  value       = proxmox_virtual_environment_vm.worker.ipv4_addresses[0]
}

// output "all_node_ips" {
//   description = "All node IP addresses"
//   value       = concat(
//     proxmox_virtual_environment_vm.control_plane.ipv4_addresses,
//     proxmox_virtual_environment_vm.worker.ipv4_addresses
//   )
// }